...
def getjourferie( dateDebut , dateFin ) :
	try :
		curseur = getConnexionBD().cursor()
		requete = '''
					select dateFerie
		...
		curseur.execute( requete , (dateDebut , dateFin ) )
...
def getdateferie() :
	try :
		...
		for unEnregistrement in enregistrements:
			uneDate = {}
			uneDate['date'] = '%04d-%02d-%02d' % ( unEnregistrement[0].year , unEnregistrement[0].month , unEnregistrement[0].day )
			uneDate['libelle'] = unEnregistrement[ 1 ]
			
			datefer.append( uneDate )
...
def deldateferie( date ) :
	
	try :
		curseur = getConnexionBD().cursor()
		
		requete = '''
					delete from JourFeries
					where dateFerie = %s
				'''
...
def creerdateferie( date , libelle ) :
	try :
		curseur = getConnexionBD().cursor()
		
		requete = '''
					insert into JourFeries
					values( %s , %s )
				'''
			
		curseur.execute( requete , ( date , libelle ) )
		connexionBD.commit()
		nbTuplesTraites = curseur.rowcount
		curseur.close()
...
def ext( matricule ):
	try :
		...
		for unEnregistrement in enregistrements:
			uneDate = {}
			uneDate['numeroCarte'] = unEnregistrement[ 0 ]
			datefer.append( uneDate )
